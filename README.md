...
tries to auto-install them on demand (pip) the first time the feature is used:
...
On Debian-based systems (ex: Kali, Ubuntu, ParrotSec) the libs are also packaged:

    # mandatory:
    sudo apt-get install python3-pycurl python3-bs4
    # optional (per feature you want to enable):
    sudo apt-get install python3-fpdf2 python3-selenium python3-gi python3-gi-cairo python3-pil python3-geoip
...
Note: some optional features also need system-level (non-pip) components:
    ...
    - '--gtk'                     : the GObject-Introspection typelibs, e.g.
                                    'gir1.2-gtk-3.0' (GTK 3), on top of PyGObject.
